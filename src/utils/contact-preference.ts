...
export type CanonicalContactPreference = 'email' | 'sms' | 'call' | 'whatsapp' | null;

/**
 * Resolves any raw preference value (from DB, HubSpot, Meta, Forms, CSV)
 * to one of the 4 canonical values or null (unspecified / não informada).
...
export function resolveCanonicalPreference(raw?: string | null): CanonicalContactPreference {
  ...
  if (!norm || norm === 'null' || norm === 'undefined' || norm === 'none' || norm === 'unspecified' || norm === 'não informada' || norm === 'nao informada') {
    return null;
  }
  ...
  // WhatsApp
  if (norm === 'whatsapp' || norm === 'whats' || norm === 'zap' || norm === 'wa' || norm.includes('whatsapp')) {
    return 'whatsapp';
  }
 ...
 * Values: 'Email', 'SMS', 'Ligação', 'WhatsApp', 'Não informada'.
 */
