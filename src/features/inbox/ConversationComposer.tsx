    ...
    <div className="border-t border-gray-100 bg-white p-4 space-y-3">
      ...
      {showOverrideConfirm && isMismatch && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3 text-xs text-amber-900 flex items-start gap-2.5 animate-in fade-in duration-200">
          ...
          <div className="flex-1 space-y-1.5">
            <p className="font-semibold leading-tight font-heading">
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              >
                {isSending ? 'Enviando...' : `Sim, Enviar via ${channel.toUpperCase()}`}
              </button>
              <button
                type="button"
      ...
      <div className="relative">
        <textarea
          ...
          placeholder={`Digite sua resposta para ${leadName}...`}
          className="w-full px-3.5 py-2.5 text-xs text-slate-900 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#08254f]/20 focus:border-[#08254f] resize-none"
        />
      </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-slate-400 font-mono">
          {channel === 'sms' ? `${body.length} caracteres` : ''}
        </span>
        <button
          type="button"
          onClick={() => handleSend(false)}
          disabled={isSending || !body.trim()}
          className="btn-crimson text-xs px-4 py-2 flex items-center gap-2"
        >
          {isSending ? (
          ...
          <span>Enviar {channel.toUpperCase()}</span>
        </button>
      </div>
    </div>
  );
