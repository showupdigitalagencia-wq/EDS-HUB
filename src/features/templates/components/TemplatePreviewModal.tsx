...
import { Mail, MessageSquare, ShieldCheck, Sparkles, Smartphone } from 'lucide-react';
        ...
        {channel === 'email' && emailData && (
          <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-xs bg-white">
            ...
            <div className="bg-slate-100/90 px-4 py-3 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-2 text-xs font-semibold text-slate-600 flex items-center gap-1.5 font-heading">
                  <Mail className="w-3.5 h-3.5 text-[#08254f]" /> Cliente de Email (Desktop)
                </span>
              </div>
              <span className="text-[10px] font-mono uppercase bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md font-bold">
                Email
              </span>
            </div>

            {/* Email Headers */}
            <div className="px-5 py-3.5 bg-slate-50/60 border-b border-slate-100 text-xs space-y-1.5">
              <div className="flex items-center gap-2">
            ...
            <div className="p-4 bg-slate-100/50">
              <iframe
                title="Email Preview"
                srcDoc={emailData.html || '<p style="color:#64748b; font-size:13px;">(Template sem conteúdo HTML)</p>'}
