          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 pt-0.5">
            ...
            {item.due_at && (
              ...
              >
                ...
                <span>Due: {new Date(item.due_at).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
              </span>
            )}
          </div>
        </div>
              ...
              {item.category !== 'completed' ? (
                <>
                  <button
                    ...
                    className="p-2 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-lg border border-slate-200 text-xs font-medium transition-colors flex items-center gap-1"
                  >
